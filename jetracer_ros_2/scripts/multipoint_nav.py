...
class Multipoint_navigation:
    def __init__(self):
        ...
        # Speak English
        self.pub_eng = rospy.Publisher("/speak", String, queue_size = 10)
    ...
    def goal_result_callback(self, msg):
        ...
        data = String()
        data = "Please get the food"
        self.pub_eng.publish(data)
        index_cus = input("Please enter your number: ")
        # Reach the target point
        if msg.status.status == 3:
            self.try_again = 1
            # Cruise the remaining points of the round
            x = self.markerArray.markers[index_cus].pose.position.x
            y = self.markerArray.markers[index_cus].pose.position.y
